...
export function AuthCard({ initialTab }: { initialTab: Tab }) {
  ...
  async function googleSignIn() {
    ...
    if (error) { setGoogleLoading(false); setError(error.message ?? "Não foi possível entrar com o Google."); }
  }
  ...
  return (
    <div className="w-full rounded-2xl border border-border bg-card p-6 shadow-sm">
      ...
      <button onClick={googleSignIn} type="button" disabled={googleLoading}
        className="flex w-full items-center justify-center gap-2 rounded-xl border border-border bg-background py-2.5 text-sm font-medium transition hover:border-primary disabled:opacity-60">
